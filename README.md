...
- Google에서 개발한 오픈 소스 원격 프로시저 호출(RPC) 프레임워크
- 다양한 프로그래밍 언어를 지원, 고성능, 경량화, 이식성 및 상호 운용성이 높은 RPC 프레임워크
- Protocol Buffers를 기반으로 하며, 기본적으로 HTTP/2 프로토콜을 사용하여 효율적인 통신을 지원
- 클라이언트-서버 애플리케이션을 빠르고 안정적으로 개발할 수 있도록 도와준다
- 또한, 여러 가지 기능을 제공하는데, 예를 들어 양방향 스트리밍, 클라이언트와 서버 간 인증 및 보안, 로드 밸런싱 등이 있다.
...
- Google의 다양한 서비스, Netflix, Square, Dropbox, CoreOS, Cockroach Labs 등이 gRPC를 사용하고 있다.
...
- 클라이언트에서는 gRPC Stub을 사용하여 서버에 메시지를 보내고, 서버에서는 gRPC Stub을 사용하여 클라이언트로부터 받은 메시지를 처리한다.
- 이러한 Stub은 프로토콜 버퍼를 기반으로 하며, 클라이언트와 서버 간에 전송되는 모든 데이터는 이진 형식으로 직렬화된다.
